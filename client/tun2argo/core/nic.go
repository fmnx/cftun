...
import (
	...
	"github.com/fmnx/cftun/client/tun2argo/core/option"
)
...
func withMulticastGroups(nicID tcpip.NICID, multicastGroups []netip.Addr) option.Option {
	return func(s *stack.Stack) error {
		...
		// The default NIC of tun2argo is working on Spoofing mode. When the UDP Endpoint
		...
		// primary addressState to build the route. However, when tun2argo is in the
